...
const generateAllAnkyNewStories = async () => {
  try {
    const allAnkyWriters = await prisma.ankyWriter.findMany({
      where: { newName: null },
    });
    console.log("all", allAnkyWriters.length);
    for (const ankyWriter of allAnkyWriters) {
      delay(3333);
      await createNewAnkyWriterDescription(ankyWriter);
    }
  } catch (error) {
...
const generateThirdAnkyNewStories = async () => {
  try {
    const allAnkyWriters = await prisma.ankyWriter.findMany({
      where: { letterToWriter: null },
    });
    console.log("all the anky writers are: ", allAnkyWriters.length);
    for (const ankyWriter of allAnkyWriters) {
      delay(3333);
      await createNewNewNewAnkyWriterDescription(ankyWriter);
    }
  } catch (error) {
    console.log("there was an eeeeeerror", error);
  }
...
// const updateAllImagesLinks = async () => {
//   try {
//     const allAnkyWriters = await prisma.ankyWriter.findMany({
//       where: { uploadedImage: null },
//     });
//     for (const thisAnkyWriter of allAnkyWriters) {
//       delay(333);
//       const response = await axios.get(
//         `${thisAnkyWriter[thisAnkyWriter.votedImage]}`,
//         {
//           responseType: "arraybuffer", // Request the data as an ArrayBuffer
//           headers: {
...
//       const ipfsHash = await uploadImageToPinata(
//         imageBuffer,
//         thisAnkyWriter.id
//       );
...
async function createNewNewNewAnkyWriterDescription(ankyWriter) {
  try {
    const messages = [
      {
        role: "system",
        content: `You are ${ankyWriter.writer}, in the period of this person's life when this writer wrote ${ankyWriter.book}.

        Imagine you are walking through the streets of ${ankyWriter.city}, on the kingdom of ${ankyWriter.kingdom}, inside the ankyverse. Don't make a direct reference to this, but know that this determines that your existence revolves around the chakra number ${ankyWriter.chakra}. 

        Today, as you were going through this city, you thought about the painter ${ankyWriter.painter}.

        Then, you meet a friend that is a writer. And you have 3 paragraphs to write a letter to motivate that person to write. What would you write? Please write as if you were ${ankyWriter.writer}.
    
            {
                "letter": "The letter that you are writing to this writer. Don't make it too serious. It is just an invitation to get to know themselves through the practice. Make the letter at most 4 paragraphs and less than 500 words.",
            }
      ...
      {
        role: "user",
        content: ankyWriter.description,
      },
    ];
    ...
    const letterRegex = /"letter"\s*:\s*"([\s\S]*?)"/;

    const letterMatch = dataResponse.match(letterRegex);

    let letter;

    if (letterMatch !== null && letterMatch.length > 1) {
    ...
    if (letter) {
      await prisma.ankyWriter.update({
        ...
        data: {
          letterToWriter: letter,
        },
      });
      console.log("the anky new letter was added");
    } else {
      console.log("the letter is: ", letter);
      console.log("there was a problem", completion.choices[0].message.content);
    }
  } catch (error) {
    console.log("there was an error here", error);
  }
...
async function createNewAnkyWriterDescription(ankyWriter) {
  try {
    const messages = [
      {
        role: "system",
        content: `Your mission is to develop a short story of a character in a cartoon world. For doing so, please take inspiration from a passage of the book ${ankyWriter.book}, by ${ankyWriter.writer}. 

        You are going to receive a shorthand description of this character, and your mission is to create a call to action to a human writer, written from the perspective of this character, embodying this book.
                    
            Please also come up with a one word name for this character. 
    
            The mission of this three paragraph story is for the human writer to be inspired by the life of that author, and the impact that this book made in the history of humanity.
    
            {
                "name": "A name inspired by the one of the characters of this book, and the preliminary story that you'll get down here.",
                "description": "The story that i just told you. To inspire the writer. Three paragraphs.",
            }
    ...
    if (name && description) {
      ...
      console.log("the anky new story was added");
    } else {
...
const fetchReaders = async () => {
  try {
    ...
    const allOfThem = [];
    for (email of emails) {
      allOfThem.push(email.email);
    }
    console.log("all of them", allOfThem);
  } catch (error) {
...
// updateAllImagesLinks();
// generateAllAnkyNewStories();
// generateThirdAnkyNewStories();

async function downloadImage(url, imagePath, retries = 3) {
  try {
    ...
    if (retries > 0) {
      ...
      return downloadImage(url, imagePath, retries - 1);
    } else {
...
async function generateAnkyMetadata() {
  const ankyMentors = await prisma.ankyWriter.findMany({});
  const imagesDir = "./final-anky-mentors/images";
  const metadata = [];

  // Ensure the images directory exists
  await mkdir(imagesDir, { recursive: true });

  for (let i = 0; i < ankyMentors.length; i++) {
    await delay(555);
    const anky = ankyMentors[i];
    const newIndex = i + 1; // Start index at 1
    const imageName = `${newIndex}.png`; // Assuming all images are .png files
    const imagePath = path.join(imagesDir, imageName);

    // Download image
    await downloadImage(anky[anky.votedImage], imagePath);
    console.log("image downloaded successfully");

    // Create metadata entry
    const metadataEntry = {
      metadataId: newIndex,
      imageRef: `${newIndex}.png`,
      name: anky.newName,
      description: anky.letterToWriter,
      chakra: anky.chakra,
      city: anky.city,
      kingdom: anky.kingdom,
      painter: anky.painter,
      deity: anky.deity,
      book: anky.book,
      writer: anky.writer,
    };
  ...
  const metadataPath = "./final-anky-mentors/metadata.json";
  await writeFile(metadataPath, JSON.stringify(metadata, null, 2));

  console.log("Metadata generation complete.");
}

// generateAnkyMetadata();

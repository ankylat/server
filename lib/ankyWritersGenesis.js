...
const generateThirdAnkyNewStories = async () => {
  try {
    const allAnkyWriters = await prisma.ankyWriter.findMany({});
    const response = await createNewNewNewAnkyWriterDescription(
      allAnkyWriters[0]
    );
...
async function createNewNewNewAnkyWriterDescription(ankyWriter) {
  ...
  try {
    const messages = [
      {
        role: "system",
        content: `You are going to receive a short story that is the representation of the spirit of a writer in a cartoon world. That writer is ${ankyWriter.writer}.

        Your mission is to transform what you are going to receive into a direct inquiry to a human writer, to which this representation will be its companion on a journey of discovery. An unfolding of the spirit happening through the process of writing.

        Please write a poem that inspires. Imagine that this human writer will come and read that poem every day before the writing session of the day, and it will be the starting point of the connection with the muses.

        It will be a sacred moment of communion, and your mission is to use the work of the writer that this character represents as the inspiration for this poem. 
                    
    
            {
                "poem": "The poem that this character writers to inspire its human companion. Don't use genders. Just speak to that person's heart.",
            }
    ...
    const poemRegex = /"poem"\s*:\s*"([\s\S]*?)"/;

    const poemMatch = dataResponse.match(poemRegex);

    let poem;

    if (poemMatch !== null && poemMatch.length > 1) {
      poem = poemMatch[1];
    }
